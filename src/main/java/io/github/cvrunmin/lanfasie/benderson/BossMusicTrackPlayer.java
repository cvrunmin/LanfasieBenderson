...
public class BossMusicTrackPlayer {
    ...
    private final float musicDuration;
    private final float crossFadeDuration;
    private final float crossFadeStartTime;
    ...
    public BossMusicTrackPlayer(Holder<SoundEvent> soundEvent, float musicDuration, float crossFadeDuration, float crossFadeStartTime){
        ...
        this.crossFadeStartTime = crossFadeStartTime;
    }
                        ...
                        if(sec >= crossFadeStartTime && sec <= crossFadeStartTime + crossFadeDuration){
                            ...
                            if(tracks[nextTrack] == null){
                                ...
                                if(playResult == SoundEngine.PlayResult.STARTED || playResult == SoundEngine.PlayResult.STARTED_SILENTLY){
                                    ...
                                    if(handle1 != null){
                                        handle1.execute(channel -> ((ISoundPositionVisitor) channel).setSoundPositionSec(sec - crossFadeStartTime));
                                    }
                                }
                            }
